...
def run_q_learning():
    """
    Run Q-Learning algorithm on the coloringNinja environment and measure its performance.
    """
    ninja_env = coloringNinja(lineSize=6)  # Initialize the environment
    print("\nRun Q-Learning")
    ...
    q_agent = QLearningAgent(ninja_env, alpha=0.1, gamma=0.9, epsilon=0.5)
    q_agent.train(episodes=500, step_limit=100)
    ...
    print(f"Q-Learning - Time: {elapsed_time:.3f} seconds")
    print(f"Q-Learning - Memory used: {memory_used:.2f} MB")

    return {"Q_table": q_agent.Q}, elapsed_time, memory_used
